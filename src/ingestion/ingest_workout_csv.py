...
# 13 canonical column names the Hevy column map must declare under `headers:`
# (spec 04 §6) — distinct from _CANONICAL_KEYS because Hevy's CSV shape
# (separate start/end timestamps, zero-based set index, cardio columns,
# per-set notes) has no equivalent in the Strong canonical set.
_HEVY_CANONICAL_KEYS = frozenset(
    {
        "workout_name",
        "started_at",
        "ended_at",
...
def _load_and_validate_all_maps(
    source_dir: Path, canonical_keys: frozenset[str] = _CANONICAL_KEYS
) -> dict[str, dict]:
    """Load and validate every *.yaml in source_dir.

    `canonical_keys` is the required key set for the `headers:` section —
    defaults to Strong's 12 keys; sources with a different CSV shape (e.g.
    Hevy) pass their own set (see _HEVY_CANONICAL_KEYS).
...
# ---------------------------------------------------------------------------
# Hevy-specific transformation helpers (spec 04 §7) — pure functions, no I/O
# ---------------------------------------------------------------------------
...
def ingest_csv(file_path: str, source: str, lang: str | None) -> dict[str, Any]:
    ...
    # cardio columns, per-set notes) has no equivalent in Strong's canonical
    # key set, so it has its own write path (spec 04).
    if source == "hevy":
        return _ingest_hevy_csv(csv_path, lang)
...
# ---------------------------------------------------------------------------
# Hevy ingestion path (spec 04)
# ---------------------------------------------------------------------------


def _ingest_hevy_csv(csv_path: Path, lang: str | None) -> dict[str, Any]:
    """Ingest a Hevy CSV export into raw.workout_sessions, raw.exercises,
    raw.sets, and raw.cardio_sets.

    Hevy's CSV shape (separate start/end timestamps, zero-based set index,
    cardio columns, per-set notes) has no equivalent in Strong's canonical
    key set, so this is a dedicated write path. Column-map loading/validation,
    decimal parsing, UUIDv5 generation, and the ingestion-log lifecycle are
    still reused from the shared helpers above.
    ...
    # Phase 1: validate all YAML maps before touching any data rows (§6.6)
    all_maps = _load_and_validate_all_maps(source_dir, canonical_keys=_HEVY_CANONICAL_KEYS)
    ...
    try:
        ...
        with csv_path.open(encoding="utf-8", newline="") as fh:
            ...
            for raw_row in dict_reader:
                ...
                canonical = _map_row_to_canonical(filtered_row, col_map)
                ...
                weight_val = _parse_decimal(canonical.get("weight_kg", ""))
                reps_raw = canonical.get("reps", "").strip()
                reps_val = int(float(reps_raw)) if reps_raw else None
                rpe_val = _parse_decimal(canonical.get("rpe", ""))
                ...
                if is_cardio_row(distance_km_raw, duration_seconds_raw):
                    distance_val = _parse_decimal(distance_km_raw)
                    duration_dec = _parse_decimal(duration_seconds_raw)
                    duration_val = int(duration_dec) if duration_dec is not None else None
                    cardio_set_id = _make_uuid5(
        ...
        with conn.cursor() as cur:
            # --- raw.workout_sessions ---
            for session_key, sess in sessions.items():
                session_id = _make_uuid5(f"{source}|{session_key}")
